...
Automatically extracting a tar file
...
Question: "Can you explain different use cases of the ADD instruction and when it's preferred over COPY?"

Answer Structure:

Basic Copying: ADD behaves like COPY for local files (we copied note.txt).
Archive Extraction: ADD can automatically extract .tar, .tar.gz, and similar archives into the image (we added archive.tar.gz which extracted index.html and style.css).
Remote URL Support: ADD can download files from the internet directly (we fetched a GitHub-hosted shell script).
Best Practice: While ADD is powerful, COPY is preferred for clarity unless archive extraction or remote download is truly needed.
